...
.이 작업에 대해
iSCSI 또는 FC 프로토콜을 사용하고 VMware Sphere용 ONTAP 도구를 사용하는 경우 선택적으로 다음을 수행할 수 있습니다. link:https://docs.netapp.com/us-en/netapp-solutions/vmware/vmware-vmsc-with-smas.html["VM ware용 ONTAP 도구를 사용하여 SnapMirror 활성 동기화를 구성합니다."^] .


NOTE: ONTAP 9.19.1부터 인공지능 기반 자율 랜섬웨어 보호(ARP/AI)는 ASA r2 시스템의 SnapMirror 액티브 싱크 SAN 관계에서 기본 읽기/쓰기 스토리지 장치를 지원합니다. ARP/AI는 활성 기본 스토리지 장치에서 실행되어 기본 전용 ARP 스냅샷을 생성하고 해당 스토리지 장치에 대한 머신 러닝 기반 랜섬웨어 탐지 기능을 제공합니다. ARP/AI를 활성화하려면 link:../secure-data/enable-anti-ransomware-protection.html["AI를 활용한 자율형 랜섬웨어 보호 기능 활성화"]을 참조하십시오.
