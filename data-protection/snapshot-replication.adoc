...
랜섬웨어 공격에 대한 보안을 강화하려면 SnapLock for SnapVault 복제 관계를 생성하십시오.
...
=== 스냅샷을 원격 클러스터로 복제하는 관계를 생성합니다.

스냅샷을 원격 클러스터에 복제하여 데이터를 보호합니다. 일관성 그룹의 복제된 스냅샷은 재해 복구 또는 데이터 마이그레이션에 사용할 수 있습니다.
...
|===
. 복제 정책을 선택합니다.
. 데이터 전송 시작을 지연시키려면 *대상 설정*을 선택한 다음 *즉시 전송 시작*을 선택 해제하십시오.
+
초기 데이터 전송은 기본적으로 즉시 시작됩니다.

. 선택적으로 기본 전송 일정을 재정의하려면 *대상 설정*을 선택한 다음 *전송 일정 재정의*를 선택하십시오.
+
...
.다음 단계
이제 복제 정책과 관계를 생성했으므로 복제 정책에 정의된 대로 초기 데이터 전송이 시작됩니다. 선택적으로 복제 페일오버를 테스트하여 ASA r2 시스템이 오프라인 상태가 될 경우 성공적인 페일오버가 발생하는지 확인할 수 있습니다.



[[create-a-relationship-to-replicate-snapshots-with-snaplock-for-snapvault]]
=== SnapLock for SnapVault로 스냅샷을 복제하는 관계 생성

랜섬웨어 공격으로부터 보호하기 위해 복제 관계를 설정할 때 볼트 복제 정책을 선택한 다음 대상 스냅샷을 잠그도록 선택하여 SnapVault용 SnapLock을 활성화할 수 있습니다. SnapVault용 SnapLock으로 보호되는 스냅샷은 실수로 또는 악의적으로 삭제할 수 없습니다.


NOTE: ASA r2 시스템은 SnapLock Compliance 모드 또는 SnapLock Enterprise 모드를 지원하지 않습니다. SnapLock for SnapVault 복제 관계를 생성하면 대상 스냅샷 잠금만 활성화됩니다. SnapLock Compliance 또는 Enterprise 모드는 활성화되지 않습니다.

.시작하기 전에
반드시 link:../secure-data/snaplock.html#initialize-the-snaplock-compliance-clock["SnapLock 규정 준수 시계를 초기화합니다"].

.단계
. 시스템 관리자에서 *보호 > 일관성 그룹*을 선택한 다음 일관성 그룹을 선택합니다.
. image:icon_kabob.gif["세 개의 수직 파란색 점"]을 선택한 다음 *보호*를 선택합니다.
. Remote protection * 아래에서 * Replicate to a remote cluster * 를 선택합니다.
. 복제 정책 * 을 선택합니다.
+
반드시 _vault_replication 정책을 선택해야 합니다.

. *대상 설정*을 선택한 다음 *삭제를 방지하기 위해 대상 스냅샷 잠금*을 선택합니다.
...
.다음 단계
이제 복제 관계를 생성했으므로 복제 정책에 정의된 대로 초기 데이터 전송이 시작됩니다. 선택적으로 복제 페일오버를 테스트하여 ASA r2 시스템이 오프라인 상태가 될 경우 성공적인 페일오버가 발생하는지 확인할 수 있습니다.
