...
---
= SnapMirror ASA r2 시스템용 SnapMirror 액티브 싱크 설정 워크플로
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
SnapMirror 액티브 동기화를 구성하기 위한 System Manager 절차는 ASA r2 시스템과 통합 ONTAP 퍼스널리티를 실행하는 NetApp FAS, AFF 및 ASA 시스템에서 서로 다르지만, SnapMirror 액티브 동기화의 요구 사항, 아키텍처 및 작동 방식은 동일합니다.link:https://www.netapp.com/pdf.html?item=/media/138366-sb-3457-san-disaster-recovery-netapp-asa.pdf["ASA r2 시스템에서 SnapMirror Active Sync를 사용한 재해 복구에 대해 자세히 알아보세요."^]

ASA r2 시스템에서 SnapMirror 액티브 동기화는 대칭형 액티브/액티브 구성을 지원합니다. 대칭형 액티브/액티브 구성에서 양쪽 사이트 모두 액티브 I/O를 위해 로컬 스토리지에 액세스할 수 있습니다. link:https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/architecture-concept.html#symmetric-activeactive["대칭 활성/활성 구성"^]에 대해 자세히 알아보십시오.

SnapLock Enterprise 및 Compliance 모드는 ASA r2 시스템에서 SnapMirror 활성 동기화를 지원하지 않습니다.

.이 작업에 대해
* ONTAP 9.19.1부터 인공지능 기반 자율 랜섬웨어 보호(ARP/AI)는 ASA r2 시스템의 SnapMirror 액티브 싱크 SAN 관계에서 기본 읽기/쓰기 스토리지 장치를 지원합니다. ARP/AI는 활성 기본 스토리지 장치에서 실행되어 기본 전용 ARP 스냅샷을 생성하고 해당 스토리지 장치에 대한 머신 러닝 기반 랜섬웨어 탐지 기능을 제공합니다. ARP/AI 활성화 단계는 link:../secure-data/enable-anti-ransomware-protection.html["AI를 활용한 자율형 랜섬웨어 보호 기능 활성화"]을 참조하십시오.
* ONTAP 9.18.1부터 SnapMirror 활성 동기화가 4노드 구성에서 지원됩니다.  ONTAP 9.17.1에서는 SnapMirror 활성 동기화가 2노드 구성에서만 지원됩니다.


.시작하기 전에
ONTAP 9.19.1 이상 버전을 실행 중이고 AIX 호스트에 대해 SnapMirror 활성 동기화를 활성화하는 경우 link:prepare-aix-snapmirror-active-sync.html["AIX 호스트를 준비합니다"]를 수행해야 합니다.
