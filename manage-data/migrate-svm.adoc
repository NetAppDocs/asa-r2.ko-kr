...
[role="lead"]
ONTAP 9.18.1 버전부터는 스토리지 가상 머신(VM)을 표준 ASA 클러스터에서 ASA r2 클러스터로 중단 없이 마이그레이션할 수 있습니다. 표준 ASA 클러스터에서 ASA r2 클러스터로 마이그레이션하면 SAN 전용 환경을 위한 ASA r2 시스템의 간소화된 아키텍처를 활용할 수 있습니다.


NOTE: ASA MetroCluster에서 ASA r2 클러스터로 스토리지 VM을 마이그레이션할 수 없습니다. ASA r2 시스템은 MetroCluster 구성을 지원하지 않습니다.
...
|===
ASA 및 ASA r2 시스템의 최신 목록은 다음을 참조하세요.link:https://hwu.netapp.com/["NetApp Hardware Universe를 참조하십시오"^] .  ASA r2 시스템은 NetApp Hardware Universe 에 "ASA A-시리즈/C-시리즈(신제품)"로 나열되어 있습니다.
