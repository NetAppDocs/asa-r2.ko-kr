...
LIF(네트워크 인터페이스)는 물리적 또는 논리적 포트와 연결된 IP 주소입니다.  데이터 액세스에 사용할 포트에 LIF를 생성하십시오.  스토리지 VM은 하나 이상의 LIF를 통해 클라이언트에 데이터를 제공합니다.  ASA r2 시스템에서는 IP, FC 및 NVMe/FC LIF를 생성할 수 있습니다.  IP 데이터 LIF는 기본적으로 iSCSI 및 NVMe/TCP 트래픽을 모두 처리할 수 있습니다.  FC 및 NVMe/FC 트래픽에는 각각 별도의 데이터 LIF를 생성해야 합니다.

.ASA r2 시스템의 LIF 페일오버
구성 요소에 장애가 발생하면 LIF는 페일오버되거나 다른 물리적 포트로 마이그레이션되어 네트워크 통신이 중단되지 않습니다.  ONTAP 9.19.1부터 IP LIF에 대해 자동 LIF 페일오버가 기본적으로 지원됩니다.  자동 LIF 페일오버가 활성화되면 스토리지 페일오버가 발생할 경우 LIF는 홈 노드 또는 포트에서 HA 파트너 노드 또는 포트로 자동으로 마이그레이션된 후 페일오버가 완료되면 다시 원래 노드 또는 포트로 돌아갑니다.  또는 포트 상태가 불안정해지면 LIF는 현재 홈 노드의 정상 포트로 자동으로 마이그레이션된 후 포트가 다시 정상화되면 원래 포트로 돌아갑니다.

ONTAP 9.18.1 이하 버전에서는 iSCSI 및 NVMe/TCP 트래픽을 모두 사용하는 IP LIF에 대해 자동 LIF 페일오버가 지원되지 않습니다. iSCSI 전용 LIF를 생성할 때, 스토리지 VM에 이미 자동 LIF 페일오버가 활성화되지 않은 iSCSI LIF가 있는 경우를 제외하고 기본적으로 자동 LIF 페일오버가 활성화됩니다. 스토리지 VM에 자동 LIF 페일오버가 활성화되지 않은 iSCSI LIF가 있는 경우, 새로 생성하는 LIF도 활성화되지 않습니다. 이러한 경우에는 기존 iSCSI LIF를 먼저 삭제한 다음 자동 LIF 페일오버를 활성화하여 다시 생성해야 합니다. link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["SAN 환경에서 LIF 삭제"^] 방법을 알아보십시오.

영구 포트는 FC 및 NVMe/FC LIF의 페일오버에 사용됩니다. 자세한 내용은 link:https://docs.netapp.com/us-en/ontap/san-config/asa-support-limitations.html#support-for-persistent-ports["영구 포트"^] ONTAP 설명서를 참조하십시오.
...
ONTAP 9.18.1 이하 버전에서 자동 iSCSI LIF 페일오버를 활성화하려면 먼저 사용자 지정 iSCSI 전용 LIF 서비스 정책을 생성해야 합니다.
...
ONTAP 9.18.1 이하 버전에서는 스토리지 VM의 다른 모든 iSCSI LIF에 대해 자동 LIF 페일오버가 활성화된 경우, iSCSI 전용 LIF를 생성할 때 자동 LIF 페일오버가 기본적으로 활성화됩니다. 스토리지 VM에 자동 LIF 페일오버가 활성화되지 않은 iSCSI LIF가 있는 경우, 새로 생성되는 LIF에도 자동 LIF 페일오버가 활성화되지 않습니다.

.시작하기 전에
사용자 지정 iSCSI 전용 LIF 서비스 정책을 만들어야 합니다.

.이 작업에 대해
iSCSI 전용 LIF를 생성하려면 ONTAP 명령줄 인터페이스(CLI)를 사용해야 합니다.
...
.이 작업에 대해
ONTAP NTP(네트워크 시간 프로토콜)를 활용하여 클러스터 전체에서 시간을 동기화합니다. LIF IP 주소를 변경한 후에는 동기화 실패를 방지하기 위해 NTP 구성을 업데이트해야 할 수도 있습니다. 자세한 내용은 지식 기반 문서를 참조하세요.link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-500629["LIF IP 변경 후 NTP 동기화가 실패합니다."] .
...
.관련 정보
* link:https://docs.netapp.com/us-en/ontap/san-admin/move-san-lifs-task.html["SAN LIF 이동"^].
* link:https://docs.netapp.com/us-en/ontap/san-admin/remove-lif-from-portset-task.html["포트셋에서 SAN LIF 제거"^].
