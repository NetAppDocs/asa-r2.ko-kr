...
구성 요소에 장애가 발생하면 LIF는 페일오버되거나 다른 물리적 포트로 마이그레이션되어 네트워크 통신이 중단되지 않습니다. ONTAP 9.19.1부터 iSCSI, NVMe/TCP 및 공유 IP LIF에 대해 자동 LIF 페일오버가 기본적으로 지원됩니다. 자동 LIF 페일오버가 활성화되면 스토리지 페일오버가 발생할 경우 LIF는 홈 노드 또는 포트에서 HA 파트너 노드 또는 포트로 자동으로 마이그레이션된 후 페일오버가 완료되면 다시 원래 노드 또는 포트로 돌아갑니다. 또는 포트 상태가 불안정해지면 LIF는 현재 홈 노드의 정상 포트로 자동으로 마이그레이션된 후 포트가 다시 정상화되면 원래 포트로 돌아갑니다.

ONTAP 9.18.1 이하 버전에서는 NVMe/TCP LIF 또는 공유 IP LIF에 대해 자동 LIF 페일오버가 지원되지 않습니다. iSCSI 전용 LIF를 생성할 때, 스토리지 VM에 이미 자동 LIF 페일오버가 활성화되지 않은 iSCSI LIF가 있는 경우를 제외하고 기본적으로 자동 LIF 페일오버가 활성화됩니다. 스토리지 VM에 자동 LIF 페일오버가 활성화되지 않은 iSCSI 전용 LIF가 있는 경우, 새로 생성하는 LIF도 활성화되지 않습니다. 이러한 경우에는 기존 iSCSI 전용 LIF를 먼저 삭제한 다음 자동 LIF 페일오버를 활성화하여 다시 생성해야 합니다. link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["SAN 환경에서 LIF 삭제"^]하는 방법을 알아보십시오.
...
ONTAP 9.18.1 이하 버전에서는 스토리지 VM의 다른 모든 iSCSI LIF에 대해 자동 LIF 페일오버가 활성화된 경우, iSCSI 전용 LIF를 생성할 때 자동 LIF 페일오버가 기본적으로 활성화됩니다. 스토리지 VM에 자동 LIF 페일오버가 활성화되지 않은 iSCSI 전용 LIF가 있는 경우, 새로 생성되는 LIF에도 자동 LIF 페일오버가 활성화되지 않습니다.
