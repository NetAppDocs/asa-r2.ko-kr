...
searchtitle: ASA r2와 다른 ONTAP 시스템 간의 차이점 
---
= ASA r2와 다른 ONTAP 시스템 간의 차이점
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
스토리지 유닛(LUN 또는 NVMe 네임스페이스 기반)이 생성되면 ONTAP은 해당 스토리지 유닛을 저장할 적절한 스토리지 가용성 영역에 볼륨을 자동으로 생성합니다. 새로 생성된 볼륨은 최적의 성능과 균형 잡힌 용량 활용을 위해 스토리지 가용성 영역 내에 자동으로 배치됩니다. 스토리지 가용성 영역 내의 용량 활용은 사용 중인 ONTAP 버전에 따라 균형을 유지합니다. link:../administer/space-balancing.html["ASA r2 클러스터에서 용량 균형 조정에 대해 알아보세요."^].
