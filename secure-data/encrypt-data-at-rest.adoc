...
온보드 키 관리자에서 외부 키 관리자로 전환하거나 그 반대로 전환해야 하는 경우, 를 참조하십시오.link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/migrate-keys-between-key-managers.html["키 관리자 간에 ONTAP 데이터 암호화 키를 마이그레이션합니다"^]
