...
* ONTAP 9.19.1부터 인공지능 기반 자율 랜섬웨어 보호(ARP/AI)는 ASA r2 시스템의 SnapMirror 액티브 싱크 SAN 관계에서 기본 읽기/쓰기 스토리지 장치를 지원합니다. ARP/AI는 활성 기본 스토리지 장치에서 실행되어 기본 전용 ARP 스냅샷을 생성하고 해당 스토리지 장치에 대한 머신 러닝 기반 랜섬웨어 탐지 기능을 제공합니다.
...
. *랜섬웨어 방지* 옆에서 image:icon_kabob.gif["랜섬웨어 방지 설정 편집"]을 선택한 다음 *기존의 모든 스토리지 유닛에서 활성화*를 선택합니다.
. *활성화*를 선택하세요.
