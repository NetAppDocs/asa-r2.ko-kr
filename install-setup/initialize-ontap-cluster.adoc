...
특정 상황에서는 다음과 같은 조치가 필요할 수 있습니다. link:../learn-more/cli-support.html#set-up-an-ontap-asa-r2-cluster-using-the-cli["ONTAP 명령줄 인터페이스(CLI)를 사용하여 클러스터를 설정하십시오."]. 예를 들어 보안 프로토콜 때문에 랩톱을 관리 스위치에 연결할 수 없거나 Windows가 아닌 운영 체제를 사용하는 경우 CLI를 사용해야 합니다.
...
. 비밀번호를 다시 입력하여 확인한 후 *계속*을 선택합니다.
