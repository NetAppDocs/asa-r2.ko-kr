...
summary: 클러스터 통신, 관리 액세스 및 SAN 호스트 연결을 활성화하려면 ASA A20, ASA A30 또는 ASA A50 스토리지 시스템을 네트워크 및 스토리지 셸프에 연결하십시오. 
---
= ASA A20, ASA A30 및 ASA A50 스토리지 시스템의 하드웨어 케이블 연결
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
클러스터 통신, 관리 액세스 및 SAN 호스트 연결을 활성화하려면 ASA A20, ASA A30 또는 ASA A50 스토리지 시스템을 네트워크 및 스토리지 셸프에 연결하십시오. 이 절차에는 클러스터/HA 인터커넥트, 관리 네트워크, 호스트 네트워크 및 스토리지 셸프 연결을 위한 케이블링이 포함됩니다.
...
+
image::../media/drw_isi_a20_switchless_2p_25gbe_cabling_ieops-2018.svg[25GbE I/O 모듈 하나를 사용한 ASA A20 스위치가 없는 클러스터 케이블링 다이어그램]
...
+
image::../media/drw_isi_a20_switched_2p_25gbe_cabling_ieops-2019.svg[25GbE I/O 모듈 1개를 사용하는 ASA A20 스위치 클러스터 케이블링 다이어그램]
...
.2포트 40/100 GbE 입출력 모듈 2개가 있는 ASA A30 또는 ASA A50
각 컨트롤러의 e2b 및 e4b 포트를 사용하여 이더넷 호스트 네트워크 스위치에 연결하십시오.
...
.4포트 10/25GbE I/O 모듈 1개가 장착된 ASA A20, A30 또는 A50
슬롯 2의 I/O 모듈에 있는 4개 포트를 모두 사용하여 이더넷 호스트 네트워크 스위치에 연결합니다.

각 컨트롤러에서 포트 e2a, e2b, e2c 및 e2d를 Ethernet 호스트 네트워크 스위치에 연결합니다.
...
.4포트 64Gb/s FC I/O 모듈 1개가 장착된 ASA A20, A30 또는 A50
슬롯 2의 FC I/O 모듈에 있는 4개 포트를 모두 사용하여 FC 호스트 네트워크 스위치에 연결하십시오.

각 컨트롤러에서 포트 2a, 2b, 2c 및 2d를 FC 호스트 네트워크 스위치에 연결합니다.
